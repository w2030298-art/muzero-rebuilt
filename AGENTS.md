...
```text
步骤 1 -> 读取 docs/progress.md
         了解当前处于哪个模块的哪个 Step。

步骤 2 -> 读取 docs/plan.md 中对应模块段落
         了解接下来要做什么。

步骤 3 -> 如 docs/references/ 下有相关参考文件，读取它们
         了解实现时需要参考的技术要点。
...
- `docs/plan.md` 是你的唯一执行依据。
- 按照 `plan.md` 中的 Step 编号逐步执行，不跳步。
- 每个 Step 中标注的文件名、类名、方法名、逻辑必须严格遵循。
- **你不做技术决策**：如果遇到 `plan.md` 未覆盖的情况，停下来向用户报告。
...
2. 执行编码。
3. 运行 Step 中指定的验证命令。
4. 验证通过：更新 `progress.md`，向用户报告，然后进入下一个 Step。
5. 验证失败：分析原因并修复后重试，最多 3 次。
6. 若 3 次后仍失败：停止并向用户报告。

### 2.3 进度更新

每完成一个 Step，立即更新 `docs/progress.md`：

```markdown
### 模块 A：[名称]
- [x] Step 1: 创建文件结构 ✅ 2025-xx-xx
- [x] Step 2: 实现数据模型 ✅ 2025-xx-xx
- [ ] Step 3: 实现核心逻辑  -> 当前
- [ ] Step 4: 编写测试
```
...
1. **不要自行决策**。
2. 将问题记录到 `docs/issues.md`：

```markdown
## Issue #[N]: [标题]
- 发现于：[模块 X] Step [N]
- 问题描述：plan.md 要求 [xxx]，但实际情况是 [yyy]
- 我的建议：[你认为可行的方案]
- 状态：待用户确认
```
...
- 不得偏离 `plan.md` 自行添加功能或重构。
- 不得删除或修改 `plan.md` 中未要求改动的文件。
- 不得在代码中引入 `plan.md` 未指定的依赖库。
- 不得跳过测试验证。
- 不得“优化” `plan.md` 中已确定的方案（除非发现明确的 bug）。
...
层级 2 - docs/plan.md + docs/progress.md
  -> 任务级上下文，确定“做什么”和“做到哪里了”。

层级 3 - docs/references/*.md
  -> 知识级上下文，提供实现参考。
...
1. 主动告知用户“建议开启新对话继续”。
2. 确保 `docs/progress.md` 已更新到最新状态。
...
### 代码风格
```text
- Formatting and naming follow the project's existing conventions.
```

### 项目特定规则
```text
- No extra project-specific constraints were provided yet.
```
